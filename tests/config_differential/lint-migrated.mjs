// Runs core's canonical config lint over config/HeadTracking.ini, the committed file, and over
// every distinct CameraUnlock.ini the differential test migrated (the folder it names as the
// argument).
//
// A migrated file writes a value, not default, on a global row where the player's imported
// value differs from what Defaults.ini gives. The lint reports that as a committed file's
// problem, so it is the one problem a migrated file may have.
...
for (const file of files) {
  for (const problem of lintCanonicalConfig(fs.readFileSync(path.join(migratedDir, file)), options)) {
    if (HOLDS_A_VALUE.test(problem)) withValues++;
...
console.log(
  `canonical config lint: the committed file and ${files.length} migrated files pass (${withValues} set a global row to a value, as an import does where the player's value is not Defaults.ini's)`,
);
