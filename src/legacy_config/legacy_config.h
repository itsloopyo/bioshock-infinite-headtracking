...
// The INI reader as the last pre-canonical build ran it, frozen. It converts a player's old
// HeadTracking.ini exactly as that build read it, so nothing in this folder is ever edited:
// a player can update from any older build, and the conversion has to read their file the
// way the build they ran did. Differences from src/config.{h,cpp} at 4ad6dad: it fills this
// frozen copy of that commit's Config rather than the runtime one, and it writes nothing -
// a missing file reads as the defaults, which is what reading the file that build would
// have created gives.
...
// LoadOrCreate as it was, reading into `cfg`. False where that build stayed dormant: the
// file could not be opened, or [General] Port is not a number or is outside 1024-65535.
// Writes nothing.
